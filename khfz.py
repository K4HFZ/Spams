...
def logo_taekyung():
    try:
        ...
        os.system("clear")
        print(f"""
{biru}╔═╗{W}┌┬┐┌─┐    {ungu}╦ ╦{W}┌┐┌┬  ┬ {abu}| {B}»{Y}⟩{W} Creator {R}:{W} Khfz
{biru}╚═╗{W}│││└─┐ ── {ungu}║ ║{W}││││  │ {abu}| {B}»{Y}⟩{W} Github {R}: \033[4;92mgithub.com/K4HFZ\033[1;0m
{biru}╚═╝{W}┴ ┴└─┘    {ungu}╚═╝{W}┘└┘┴─┘┴ {abu}| {B}»{Y}⟩{W} Team {R}:{G} NO TEAM ( SOLO PLAYER )
